		...
		base.render = function(noty) {
			...
			if ($.noty.available) {
				...
				if (jQuery.type(notification) === 'object') {
					...
					base.$close 	= $('<div/>').addClass('noty_close').html('&times;');
					...
					if (notification.options.buttons) {
						...
						$.each(notification.options.buttons, function(i, button) {
							...
							$('<button/>').addClass(bclass).html(button.text).appendTo($noty.find('.noty_buttons')).on("click", button.click);
						});
					}
					
					// Start the show
					$noty.show().css({opacity:0}).stop(true, true).animate(
                            {opacity:1},
							'100',
